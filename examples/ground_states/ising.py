...
SR_ADAPTIVE_TARGET_FS_NORM = 0.01
...
def benchmark_output_dir() -> Path:
    ...
    return Path(__file__).resolve().parent / f"ising_benchmark_{L}x{L}_{SR_FIXED_STEPS}"
...
def write_figures(output_dir: Path) -> None:
    ...
    plot_errorbars_step(
        ...
        "mx_error",
        "m_x",
        output_dir / "mx_vs_step_errorbars.png",
    )
    plot_errorbars_step(
        plt,
        outputs,
        ...
        "zz_error",
        "Nearest-neighbor zz",
        output_dir / "zz_vs_step_errorbars.png",
    )
