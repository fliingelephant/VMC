...
class GIPEPSConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.is_binary_occupancy_matter:
            if self.conserve_particle_number:
                ...
                if self.particle_number < 0 or self.particle_number > n_sites:
                    raise ValueError(
                        "particle_number must satisfy 0 <= particle_number <= n_sites."
                    )
        ...
        if all(d == dmax for d in self.degeneracy_per_charge):
            ...
            return
        mask = tuple(
            tuple(i < d for i in range(dmax)) for d in self.degeneracy_per_charge
        )
    ...
    def is_binary_occupancy_matter(self) -> bool:
        return self.N == 2 and self.phys_dim == 2 and self.charge_of_site == (0, 1)
...
class GIPEPS(nnx.Module):
        ...
        *,
        ...
        for r in range(n_rows):
            ...
            for c in range(n_cols):
                ...
                num_links = (
                    int(r > 0) + int(r < n_rows - 1) + int(c > 0) + int(c < n_cols - 1)
                )
    ...
    def random_physical_configuration(
        ...
        if (
            self.config.is_binary_occupancy_matter
            and self.config.conserve_particle_number
        ):
            return jax.vmap(
...
def _random_plaquette_background(
    ...
    deltas = jax.random.randint(key, (n_rows - 1, n_cols - 1), 0, N, dtype=jnp.int32)
...
def _single_z2_hardcore_configuration_with_particles(
    ...
    sites = (
        jnp.zeros((n_sites,), dtype=jnp.int32)
        .at[occupied]
        .set(1)
...
def _single_z2_hardcore_configuration_with_parity(
    ...
    sites = jnp.concatenate([sites_flat, last_site[None]], axis=0).reshape(
        (n_rows, n_cols)
    )
    h_links, v_links = _random_plaquette_background(key_bg, n_rows, n_cols, 2)
    h_links, v_links = _repair_z2_gauss_law(key_pair, h_links, v_links, sites, Qx)
...
def _repair_z2_gauss_law(
    ...
    def flip_pair(carry, pair):
        ...
        return jax.lax.cond(
            valid, apply_pair, lambda _: (h_cur, v_cur), operand=None
        ), None
...
def assemble_tensors(
    ...
    for r in range(n_rows):
        ...
        for c in range(n_cols):
            row.append(
                _assemble_site(tensors, h_links, v_links, config, r, c, mask_per_charge)
            )
        eff.append(row)
    return eff
...
def _assemble_site(
    ...
    cfg_idx = _site_cfg_index(config, k_l=k_l, k_u=k_u, k_r=k_r, k_d=k_d, r=r, c=c)
...
def _horizontal_hardcore_hop(
    ...
    h_prop = h_links.at[r, c].set(jnp.where(allowed, 1 - h_links[r, c], h_links[r, c]))
...
def _vertical_hardcore_hop(
    ...
    v_prop = v_links.at[r, c].set(jnp.where(allowed, 1 - v_links[r, c], v_links[r, c]))
...
def _eval_term(
    ...
    def eval_hop(_):
        sites_prop, h_prop = _horizontal_hardcore_hop(
...
def _eval_term(
    ...
    def _flip_amp(delta):
        h_f, v_f = _plaquette_flip(
            envs.h_links, envs.v_links, row, col, delta=delta, N=envs.config.N
        )
        mpos = []
    ...
    for row in range(n_rows - 1, -1, -1):
        ...
        def _eval_dr2(
            ...
            if next_row_mpo is None:
                raise NotImplementedError(
                    "Missing next-row MPO for GI dr=2 evaluation."
                )
...
def _plaquette_sweep_row_pair(
    ...
    right_envs = _compute_right_envs_2row(
        top_env, row_mpo0, row_mpo1, bottom_env, dtype
    )
    ...
    for c in range(n_cols - 1):
        ...
        h_prop, v_prop = _plaquette_flip(
            h_links, v_links, r, c, delta=delta, N=config.N
        )
...
def _horizontal_link_sweep_row(
    ...
    for c in range(n_cols - 1):
        ...
        eff0 = _assemble_site(tensors, h_prop, v_links, config, r, c, mask_per_charge)
        eff1 = _assemble_site(
...
def _vertical_link_sweep_row_pair(
    ...
    for c in range(n_cols):
        ...
        eff0 = _assemble_site(tensors, h_links, v_prop, config, r, c, mask_per_charge)
        eff1 = _assemble_site(
...
def transition(
    ...
    if n_rows > 1:
        ...
        for r in range(n_rows - 1):
            ...
            key, row_mpo0, row_mpo1, h_links, v_links, amp_cur = (
    ...
    if config.phys_dim == 1:
        ...
        return (
            GIPEPS.flatten_sample(sites, h_links, v_links),
            key,
            amp,
            tuple(top_envs_cache),
        )
    ...
    for r in range(n_rows - 1):
        ...
        if number_conserving:
            key, row_mpo0, row_mpo1, sites, v_links, amp_cur = (
                _vertical_hardcore_hop_sweep_row_pair(
                    key,
                    tensors,
                    sites,
                    h_links,
